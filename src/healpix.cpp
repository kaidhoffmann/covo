...
healpix::Tablefiller::Tablefiller()
{
    ...
    {
        ctab[m] =
            (m & 0x1) | ((m & 0x2) << 7) | ((m & 0x4) >> 1) | ((m & 0x8) << 6) | ((m & 0x10) >> 2) | ((m & 0x20) << 5) | ((m & 0x40) >> 3) | ((m & 0x80) << 4);
        utab[m] =
            (m & 0x1) | ((m & 0x2) << 1) | ((m & 0x4) << 2) | ((m & 0x8) << 3) | ((m & 0x10) << 4) | ((m & 0x20) << 5) | ((m & 0x40) << 6) | ((m & 0x80) << 7);
    }
}
...
void healpix::SetNside(int nside)
{
    ...
    if (order_ < 0)
        std::cout << "#### WARNING: SetNside: nside must be power of 2 for nested maps ####" << std::endl;
    nside_ = nside;
...
{
    if (nside < 1)
        std::cout << "invalid value for Nside" << std::endl;
    if ((nside) & (nside - 1))
        return -1;
    return ilog2(nside);
}
