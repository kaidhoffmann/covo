    ...
    else
    {
        ...
        while (std::getline(indata, line))
        {
            ...
            while (std::getline(ss, var, p.delim_in[0]))
            {
                ...
                if (found_pos != std::end(p.cols_pos))
                {
                    obj_line.pos.push_back(std::stod(var));
                }
                if (found_vec_a != std::end(p.cols_vec_a))
                {
                    obj_line.vec_a.push_back(std::stod(var));
                }
                if (found_vec_b != std::end(p.cols_vec_b))
                {
                    obj_line.vec_b.push_back(std::stod(var));
                }
...
bool vol_lim_diff(const parameters p, const catalogue &cat_1, const catalogue &cat_2, double diff_toll)
{
    ...
    for (int i = 0; i < dim; i++)
    {
        ...
        if (D_lo / L1 > diff_toll)
            lim_diff = true;
        if (D_lo / L2 > diff_toll)
            lim_diff = true;
        if (D_hi / L1 > diff_toll)
            lim_diff = true;
        if (D_hi / L2 > diff_toll)
            lim_diff = true;
    }
...
void catalogue::make_samples_box(const parameters p)
{
    ...
    int Nsamp = 1;
    for (int i = 0; i < p.numb_jk_cart.size(); i++)
    {
        Nsamp *= p.numb_jk_cart[i];
    }
...
void catalogue::make_samples_shell(const parameters p)
{
    ...
    for (int i = 0; i < hp.cells.size(); i++)
    { // loop over healpix cells
        if (!hp.cells[i].masked)
        {
            for (int j = 0; j < p.nrad; j++)
    ...
    for (int samp_ID = 0; samp_ID < samp.size(); samp_ID++)
    {
        ...
        if (samp[samp_ID].obj.size() > 0)
        { // loop over samples

            std::vector<std::vector<double>> pos_limits;

            // initilize
            for (int dim = 0; dim < samp[samp_ID].obj[0].pos.size(); dim++)
            ...
            for (int obj_ID = 0; obj_ID < samp[samp_ID].obj.size(); obj_ID++)
            { // loop over objects in sample

                for (int dim = 0; dim < pos_limits.size(); dim++)
                { // loop over dimensions
...
void catalogue::write_input(const parameters p, const std::string filename)
{
    ...
    for (int i = 0; i < input.obj.size(); i++)
    {
        ...
        outdata << input.obj[i].pos[0] << d << input.obj[i].pos[1] << d << input.obj[i].pos[2] << d << input.obj[i].vec_a[0] << d << input.obj[i].vec_a[1] << d << input.obj[i].vec_a[2] << d << input.obj[i].vec_b[0] << d << input.obj[i].vec_b[1] << d << input.obj[i].vec_b[2];
        if (i < input.obj.size() - 1)
        {
            outdata << std::endl;
        }
    }
