...
bool directory_exists(const short verbose, const std::string pathname)
{
    ...
    {
        if (verbose > 2)
            std::cerr << "##### [io_data: directory_exists] WARNING: can't find " << pathname << " #####" << std::endl;
    }
    ...
    {
        if (verbose > 2)
            std::cerr << "##### [io_data: directory_exists] WARNING: " << pathname << "is no directory #####" << std::endl;
    }
    return dir_ex;
}
...
void make_directory(const short verbose, const std::string pathname)
{
    ...
    if (verbose > 2)
        std::cout << "##### [io_data: make_directory]: make " << pathname << " #####" << std::endl;
